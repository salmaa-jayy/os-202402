...
int main() {
   ...
   d:	57                   	push   %edi
   e:	56                   	push   %esi
   f:	8d bd e8 fb ff ff    	lea    -0x418(%ebp),%edi
  15:	8d b5 e8 fa ff ff    	lea    -0x518(%ebp),%esi
  1b:	53                   	push   %ebx
  struct pinfo p;
  getpinfo(&p);
  1c:	8d 9d e8 f9 ff ff    	lea    -0x618(%ebp),%ebx
int main() {
  22:	51                   	push   %ecx
  23:	81 ec 14 06 00 00    	sub    $0x614,%esp
  getpinfo(&p);
  29:	53                   	push   %ebx
  2a:	e8 44 03 00 00       	call   373 <getpinfo>
  ...
  4e:	57                   	push   %edi
  for(int i = 0; i < 64 && p.pid[i]; i++) {
  4f:	83 c7 10             	add    $0x10,%edi
    printf(1, "%d\t%d\t%s\n", p.pid[i], p.mem[i], p.name[i]);
  52:	ff b3 fc 00 00 00    	pushl  0xfc(%ebx)
  58:	50                   	push   %eax
  59:	68 b6 07 00 00       	push   $0x7b6
  5e:	6a 01                	push   $0x1
  60:	e8 db 03 00 00       	call   440 <printf>
  for(int i = 0; i < 64 && p.pid[i]; i++) {
  65:	83 c4 20             	add    $0x20,%esp
  68:	39 f3                	cmp    %esi,%ebx
  6a:	74 06                	je     72 <main+0x72>
  6c:	8b 03                	mov    (%ebx),%eax
  6e:	85 c0                	test   %eax,%eax
  70:	75 d6                	jne    48 <main+0x48>
  }
  exit();
